## INSTALAR DEPENDENCIAS
Para la correcta ejecucion del proyecto es necesario tener instalado un entorno que soporte notebooks (**Jupyterlab** o **VS Code**)

Si se ejecuta desde VsCode instalar la extension "Jupyter " y el paquetede ipykernel, esto sirve para visualizar los archivos .ipynb

## EJECUCION DEL PROGRAMA
1.Descargar o clonar la carpeta notebooks del repositorio
 **git clone https://github.com/MezaJoaq/programa_jupyter_notebook**
2.Entrar a la carpeta notebooks
3.Abrir el archivo llamado 10_simulacion_cocina.ipynb
4.Ejecutar cada celda en orden.